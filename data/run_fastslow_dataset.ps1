# Generate the fast-slow-fast ball dataset (50 videos) using Kubric Docker.
#
...
$OutDir     = Join-Path $ScriptDir "output\fastslow"
$Image      = "kubricdockerhub/kubruntu"

New-Item -ItemType Directory -Force -Path $OutDir | Out-Null

Write-Host "=== Generating fast-slow-fast dataset (50 videos) ===" -ForegroundColor Cyan
...
docker run --rm `
    ...
    $Image `
    /usr/bin/python3 /kubric_scripts/fastslow_ball_dataset.py `
        --output_dir /output `
        --width 256 `
        --height 256
