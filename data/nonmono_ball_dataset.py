"""
Non-monotonic ball dataset generation using Kubric.

50 videos: 8 directions × 6-7 starting positions.
Speed profile is fixed and non-monotonic for every video:
  frames  0–4  : slow  (1.5 m/s  ≈ 2 px/frame at 256×256)
  frames  5–10 : fast  (6.0 m/s  ≈ 8 px/frame at 256×256)
  frames 11–15 : slow  (1.5 m/s)
...
Usage (inside kubric Docker container):
  python3 nonmono_ball_dataset.py --output_dir /output/nonmono
"""
...
# ---------------------------------------------------------------------------
# Directions (same as velocity/acceleration datasets)
# ---------------------------------------------------------------------------
DIRECTIONS = [
...
#   frames  0– 4: slow  (5 frames)
#   frames  5–10: fast  (6 frames)
#   frames 11–15: slow  (5 frames)
FRAME_SPEEDS = [SLOW_SPEED]*5 + [FAST_SPEED]*6 + [SLOW_SPEED]*5

# Total displacement (m): 5*(1.5/24) + 6*(6.0/24) + 5*(1.5/24) = 0.3125+1.5+0.3125 = 2.125
TOTAL_DISP_M = sum(s / 24 for s in FRAME_SPEEDS)  # ≈ 2.125 m
...
# ---------------------------------------------------------------------------
# Scene constants (match velocity/acceleration datasets)
# ---------------------------------------------------------------------------
BALL_RADIUS = 0.3
BALL_MASS   = 1.0
FLOOR_HALF  = 4.0
...
def run_nonmono_speed(simulator, scene, ball, direction_xyz):
    """
    Simulate non-monotonic speed by calling resetBaseVelocity at each frame.
...
def generate_video(dir_idx, pos_idx, video_idx, output_dir, resolution):
    ...
    scratch = pathlib.Path(tempfile.mkdtemp(prefix="kubric_nonmono_"))
    ...
    # Overhead perspective camera (same as velocity/acceleration datasets)
    ...
    # Run non-monotonic speed simulation
...
def write_dataset_docs(output_dir):
    ...
    with open(readme_path, "w") as f:
        f.write("# Non-Monotonic Ball Dataset\n\n")
        f.write("Generated with Kubric. Fixed non-monotonic speed profile; "
                "direction and start-position vary.\n\n")
        f.write("## Speed profile\n\n")
        f.write("| Phase | Frames | Speed |\n|---|---|---|\n")
        f.write(f"| Slow | 0–4   | {SLOW_SPEED} m/s (~2 px/frame at 256×256) |\n")
        f.write(f"| Fast | 5–10  | {FAST_SPEED} m/s (~8 px/frame at 256×256) |\n")
        f.write(f"| Slow | 11–15 | {SLOW_SPEED} m/s |\n\n")
        f.write(f"Total displacement: ≈ {TOTAL_DISP_M:.3f} m per video.\n\n")
        f.write("## Scene setup\n\n")
        f.write("Identical to velocity/acceleration datasets: PerspectiveCamera at z=10 m, "
