"""
Temporal SAE feature analysis for the non-monotonic ball dataset.
...
(frames 0-4 slow, 5-10 fast, 11-15 slow) averaged over the 2 frames per tubelet:
  t=0: (slow+slow)/2 = slow
  t=1: (slow+slow)/2 = slow
  t=2: (slow+fast)/2 = mixed
  t=3: (fast+fast)/2 = fast
  t=4: (fast+fast)/2 = fast
  t=5: (fast+slow)/2 = mixed
  t=6: (slow+slow)/2 = slow
  t=7: (slow+slow)/2 = slow
...
Usage:
  python analyze_nonmono_temporal.py \\
    --dataset_dir /path/to/nonmono \\
    --sae_path    /path/to/ae.pt \\
    --sae_type    batch_top_k \\
    --output_path nonmono_btk.pkl \\
    --top_k       50
"""
...
# Non-monotonic speed profile (m/s per frame, matching nonmono_ball_dataset.py)
SLOW_SPEED = 1.5
FAST_SPEED = 6.0
_FRAME_SPEEDS = [SLOW_SPEED]*5 + [FAST_SPEED]*6 + [SLOW_SPEED]*5  # 16 values
...
# Dataset
# ---------------------------------------------------------------------------
class NonmonoDataset(Dataset):
    """Loads all dir{d:02d}_pos{p:02d}/ videos from a nonmono dataset directory."""
...
def parse_args():
    ...
    p.add_argument("--dataset_dir",   required=True,
                   help="Path to the nonmono/ output directory")
...
def main():
    ...
    # ── Dataset ───────────────────────────────────────────────────────────
    ds = NonmonoDataset(args.dataset_dir)
