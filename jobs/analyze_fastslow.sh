#!/bin/bash
#SBATCH --job-name=fastslow_analysis
#SBATCH --output=/net/scratch/renqy/logs/fastslow_analysis_%j.out
#SBATCH --error=/net/scratch/renqy/logs/fastslow_analysis_%j.err
...
FASTSLOW_DIR=~/sae-for-vlm/ball_dataset/fastslow
...
for SAE_TYPE in batch_top_k standard; do
  ...
  echo "=== fastslow / ${SAE_TYPE} ==="
  python analysis/analyze_fastslow.py \
    --dataset_dir "${FASTSLOW_DIR}" \
    --sae_path    "${SAE_PATH}" \
    --sae_type    "${SAE_TYPE}" \
    --output_path "${OUT_DIR}/fastslow_${TAG}.pkl" \
    --model_name  "${MODEL}" \
    --layer       11 \
    --top_k       100 \
    --batch_size  8 \
