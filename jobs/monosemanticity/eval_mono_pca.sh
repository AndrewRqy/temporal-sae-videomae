#!/bin/bash
#SBATCH --job-name=mono_score_pca
#SBATCH --output=/home/renqy/logs/mono_score_pca_%j.out
#SBATCH --error=/home/renqy/logs/mono_score_pca_%j.err
...
PCA_PATH="/net/scratch2/renqy/linear_decomp/pca.pt"

# Sign-handling mode for signed PCA components:
#   sign_split (primary) -> ReLU(+c)/ReLU(-c), 768 -> 1536 features
#   abs        (robustness) -> |c|, 768 features
# Run once per mode; outputs go to separate dirs so both are kept.
MODE="${1:-sign_split}"

# Prerequisites:
#   - DINOv2 embeddings at EMBED_PATH (jobs/dataset/run_ssv2_dino_embeddings.sh)
#   - Fitted PCA at PCA_PATH         (jobs/monosemanticity/fit_pca_ica.sh)
...
# Extract max-pooled PCA-component activations (one vector per clip)
python save_activations.py \
    ...
    --batch_size       "${BATCH}" \
    --num_workers      "${WORKERS}" \
    --output_dir       "${ACT_DIR}/pca_${MODE}_val" \
    --max_pool \
    --sae_model        pca \
    --sae_path         "${PCA_PATH}" \
    --decomp_mode      "${MODE}" \
...
python eval/metric.py \
    --activations_dir "${ACT_DIR}/pca_${MODE}_val" \
