#!/bin/bash
#SBATCH --job-name=decel_analysis
#SBATCH --output=/net/scratch/renqy/logs/decel_analysis_%j.out
#SBATCH --error=/net/scratch/renqy/logs/decel_analysis_%j.err
...
for SAE_TYPE in batch_top_k standard; do
  ...
  echo "=== deceleration / ${SAE_TYPE} ==="
  python analysis/analyze_vel_accel.py \
    --dataset_type  acceleration \
    --dataset_dir   "${BALL_DIR}/acceleration" \
    --sae_path      "${SAE_PATH}" \
    --sae_type      "${SAE_TYPE}" \
    --output_path   "${OUT_DIR}/deceleration_temporal_${TAG}.pkl" \
    --nonmono_pkl   "${NM_PKL}" \
    --reverse_frames \
    --model_name    "${MODEL}" \
    --layer         11 \
